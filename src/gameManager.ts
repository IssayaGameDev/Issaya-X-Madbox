...
export class GameManager {
  ...
  unlockLevel() {
    ...
    //Visual feedback for unlocked levels
  ...
  touchStartHandler(e: MouseEvent | TouchEvent) {
    ...
    if ("touches" in e) {
      this.input.startPosition.x =
        (e.touches[0].clientX / window.innerWidth) * 2 - 1;
      this.input.startPosition.y =
  ...
  touchMoveHandler(e: MouseEvent | TouchEvent) {
    if ("touches" in e) {
      this.input.currentPosition.x =
        (e.touches[0].clientX / window.innerWidth) * 2 - 1;
      this.input.currentPosition.y =
        (e.touches[0].clientY / window.innerHeight) * 2 + 1;
    } else {
    ...
    //Taking mouse offset and addin the X value to player velocity

    if (this.input.isPlayerTouch && this.level?.state == 1) {
      this.input.offset.x =
        this.input.currentPosition.x - this.input.startPosition.x;

      this.level?.player.threeBody?.body.velocity.set(
        -this.input.offset.x * 20,
        this.level?.player.threeBody?.body.velocity.y,
        this.level?.player.threeBody?.body.velocity.z
      );
    }
  }
  ...
  cameraUpdate() {
    ...
    //Camera Controller 
    //If in the menu look at the player from the side
    //If playing from behind
    //If dead take center of all cubes and look at it
    ...
    if (this.level?.player) {
      if (this.level.state == 1) {
        ...
        this.camera.position.lerp(
          new Vector3()
            .copy(this.level.player.position)
            .add(new Vector3(0, 3, -10)),
          0.1
        );
        ...
        for (
          let index = 0;
          index < this.level.player.cubeBodies.length;
          index++
        ) {
        ...
        this.camera.position.lerp(
          new Vector3().copy(theCenter).add(new Vector3(0, 3, -10)),
          0.1
        );
        this.camera.lookAt(theCenter);
      } else {
        this.camera.position.lerp(new Vector3().set(-5, 5, 0), 0.1);
        this.camera.lookAt(this.level.player.position);
      }
    }
  ...
  updatePhysics() {
    ...
    for (var i = 0; i < this.physicsBodies.length; i++) {
      ...
      if (physicBody.object != null && physicBody.object.parent != null) {
        physicBody.object.matrixWorld.decompose(possition, quaternion, scale);
        ...
        if (!physicBody.trigger) {
          ...
          let position = physicBody.object.parent.worldToLocal(
            new Vector3(
              physicBody.body?.position.x,
              physicBody.body?.position.y,
              physicBody.body?.position.z
            )
          );
          ...
          physicBody.object.quaternion.copy(physicBody.body.quaternion as any);
        } else if (physicBody.trigger) {
          physicBody.body.position.copy(
            new CANNON.Vec3(possition.x, possition.y, possition.z)
          );
          physicBody.body.quaternion.copy(
            new CANNON.Quaternion(quaternion.x, quaternion.y, quaternion.z)
          );
        }
      }
    }
